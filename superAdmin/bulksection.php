...
function result($output,$action = 'commit'){
    ...
    header('Location: ../superAdmin/createDepartment.php');
    exit();
}
        ...
        if(is_uploaded_file($_FILES['csv']['tmp_name'])){
           ...
           // fgetcsv($csvFile);
            ...
            while (($line = fgetcsv($csvFile, 0, ",")) !== FALSE) { //get data per row
                ...
                try{
                    $inserted = mysqli_query($con,"INSERT  INTO tblsection (levelId,sectionName) VALUES('".$line[0]."','".$line[1]."') ON DUPLICATE KEY UPDATE levelId=VALUES(levelId), sectionName = VALUES(sectionName)");
