...
if(isset($_GET['cardid'])){
    ...
    $query =mysqli_query($con, "SELECT p.studentId,p.sessionId,s.firstName,s.middleName,s.lastName,s.Id FROM tblpromoted as p LEFT JOIN (SELECT Id, firstName,middleName,lastName  FROM tblstudent) as s ON p.studentId = s.Id WHERE p.studentId='".mysqli_real_escape_string($con,$_GET['cardid'])."' AND p.sessionId = '".$_SESSION['global_session']."'");
...
if(isset($_POST['attendance'])){
   ...
   $sum = $day_present + $day_absent;
   
    $query=mysqli_query($con,"SELECT * FROM tblattendance where studentId ='".$_GET['cardid']."', month = '".$month."' , sessionId = $activesession");
   ...
   VALUES('".$_GET['cardid']."','".$month."','".$schoolday."','".$day_present."','".$day_absent."','".$activesession."','".$dateCreated."') ON DUPLICATE KEY UPDATE schoolday= VALUES(schoolday), day_present= VALUES(day_present), day_absent= VALUES(day_absent)");
                        ...
                        <div class="card-body">
                            ...
                            <div class="table-reponsive">
                                <table  class="table table-hover table-striped table-bordered">
                                        ...
                                        <tbody>
                                        <?php
                                            $query=mysqli_query($con,"SELECT * FROM tblattendance WHERE studentId = '".$_GET['cardid']."' AND sessionId = '".$_SESSION['global_session']."' ORDER BY dateCreated");
                                        ...
                                        <td>
                                            <!--<a class="editbtn" href="editattendance.php?editattendance=<?php echo $row['Id'];?>" title="Edit Details" data-bs-toggle="modal" data-bs-target="#editattendance"><i class="las la-edit"></i></a>-->
                                            <a class="delete" data-href="deleteattendance.php?delid=<?php echo $row['Id'];?>&viewId=<?php echo $_SESSION['viewId'];?>&aId=<?php echo $_GET['aId'];?>&cardid=<?php echo $_SESSION['cardid'];?>" title="Delete Student Details"><i class="las la-trash-alt"></i></a>
                                        </td>
                                        </tr>
                                        <?php
                                    }
...
<div class="modal fade" id="editattendance" tabindex="-1" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
  <div class="modal-dialog ">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="col-12 modal-title text-center" id="exampleModalLongTitle" >EDIT ATTENDANCE</h5>
      </div>
      <form method="POST" action="" >
      <div class="modal-body">
        <div class="mb-3">
            <label for="cc-exp" class="form-label">Month:</label>
            <select Required id="months" name="month"  class="form-select">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
        <button type="submit" class="btn btn-primary" name="editattendance" id="send">Edit</button>
      </div>
        </form>
    </div>
  </div>
